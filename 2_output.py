...
from qiskit.visualization import plot_histogram # Optional: Qiskit's built-in tool
...
# --- STEP 6: VISUALIZE CIRCUIT AND OUTPUT IN MATPLOTLIB ---
...
# 2. Visualize the Output Counts using pure Matplotlib
states = list(counts.keys())
frequencies = list(counts.values())

plt.figure(figsize=(8, 6))
plt.bar(states, frequencies, color='skyblue', edgecolor='black')
plt.xlabel('Measured State (Binary)', fontsize=12)
plt.ylabel('Frequency (Shots)', fontsize=12)
plt.title('Quantum Phase Estimation Output Histogram', fontsize=14)
plt.grid(axis='y', linestyle='--', alpha=0.7)

# Add exact numbers on top of each bar
for i in range(len(states)):
    plt.text(i, frequencies[i] + 15, str(frequencies[i]), ha='center', fontsize=10)

plt.show()

# Alternatively, you can use Qiskit's built-in histogram which uses matplotlib:
plot_histogram(counts, title="Quantum Phase Estimation Output")
plt.show()